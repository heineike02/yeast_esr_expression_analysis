...
# base_dir will have value /Users/greder/Dropbox/UCSF/El-Samad/expression_broad_data
...
def main():
    ...
    for jj in range(len(species_list)):
        ...
        fname = os.path.normpath(base_dir + '/scripts/expression_broad_data_datafiles/microarray_data/raw_exp/' + fname_out_bases[jj] + '_raw_exp.csv')
        ...
        fname = os.path.normpath(base_dir + '/scripts/expression_broad_data_datafiles/microarray_data/GSE36253_Growth/'  + fname_out_bases[jj] + '_growth.csv' )
        ...
        if species != 'Saccharomyces bayanus':
            ...
            fname = os.path.normpath(base_dir + '/scripts/expression_broad_data_datafiles/microarray_data/GSE38478_Stress/'  + fname_out_bases[jj] + '_stress.csv' )
        ...
        promoter_choice_fig_fname =  os.path.normpath(base_dir + '/output/' +
                                    time.strftime('%Y') + '_' + 
                                    time.strftime('%m') + '_' + 
                                    time.strftime('%d') + '_' + fname_out_bases[jj] + 
                                    promoter_choice_fig_fname_suffix )

        fig1, ax1, condition_arrays = expression_plots.promoter_choice_plot(mean_gene_expression, 
                                    condition_arrays, pct_expression, pct_stability, weight_vector, 
                                    sort_order = sort_order, N_plotted = N_plotted)
        ...
        #Tracer()()
        fig2, ax2 = expression_plots.promoter_ortholog_plot(SC_genenames, species, 
                                                            native_orfs_empirical, 
                                                            condition_arrays)
        ...
        promoter_ortholog_fig_fname =  os.path.normpath(base_dir + '/output/' +
                            time.strftime('%Y') + '_' + 
                            time.strftime('%m') + '_' + 
                            time.strftime('%d') + '_' + fname_out_bases[jj] + 
                            promoter_ortholog_fig_fname_suffix )
